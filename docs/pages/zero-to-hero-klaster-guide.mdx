...
<iframe width="640" height="416" src="https://www.loom.com/embed/e226679dc2c2494bbbf4e880937e3037?sid=ab3b4aae-5cb6-4d85-af70-4ac240c9e296" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
...
For this tutorial, we will be using [Across]('https://across.to').
In your project, create a new file and give it a name symbolizing its holding the Across bridge plugin.

In order to createa a new BridgePlugin, you can reference this code snippet:
...
Here, a lot of boilerplate code is hidden behind the `encodeAcrossCallData` and `getAcrossSuggestedFees` functions. You can learn
how to use Across for bridging by consulting their documentation.

For the purposes of this tutorial, you can copy/paste the entire file. However, there are some prerequisites if you will
be using this file - and that is installing Axios.

```bash
npm install axios
```

```ts filename="across-bridge-plugin.ts"
...
For most cases, the SDK will be able to calulate te exact routes for bridigng to get your desired outcome. If this is not
possible, you can always encode them yourself.

The function that does the calcualations is called `encodeBridgingOps`. Simply pass all of the required parameters and as
an output you'll get a series of steps (EVM Transactions) to be executed on one or more blockchains to get your desired `amoun`
